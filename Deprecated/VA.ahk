...
; MASTER CONTROLS
;
;modified VA_IAudioEndpointVolumeCallback_OnNotify
;Sets the volume of the window name,pid,id supplied returns the volume if no volume is supplied
;Rough implementation doesnt work for multi-process application like discord, chrome etc tho theoretically you could just mute every process of the specific exe
;pass 0 in vol to mute/unmute (optional)
GetProcessName(winName := "A") {
    ; If it's a number, treat as PID
    if winName is digit 
	{
        Process, Exist, %winName%
        if (ErrorLevel) {
            WinGet, name, ProcessName, ahk_pid %winName%
            return name
        }
        return
    }
    ...
    ; Try as window title
    WinGet, name, ProcessName, %winName%
    if name !=
        return name

    ; Try as AHK window ID
    WinGet, name, ProcessName, ahk_id %winName%
    if name !=
        return name
...
GetSetAppMute(appName, mute := "")
{
    appName := GetProcessName(appName)
    svclPath := "C:\Users\Share\OneDrive\Documents\AutoHotkey\svcl.exe"

    if (mute != "")
    {
        if (mute = "toggle")
            RunWait, %ComSpec% /c ""%svclPath%" /Switch "%appName%"", , Hide
        else if (mute = 1)
            RunWait, %ComSpec% /c ""%svclPath%" /Mute "%appName%"", , Hide
        else if (mute = 0)
            RunWait, %ComSpec% /c ""%svclPath%" /Unmute "%appName%"", , Hide
        Sleep, 100
    }

    output := MOD_JEE_RunGetStdOut("""" svclPath """ /Stdout /GetMute """ appName """")

    ; Output is either 1 or 0
    value := Trim(output)
    return (value = "0" or value = "1") ? value : -1
}

GetSetAppVolume(appName, vol := "")
{
    appName := GetProcessName(appName)
    svclPath := "C:\Users\Share\OneDrive\Documents\AutoHotkey\svcl.exe"

    ; Always get volume first before changing it
    output := MOD_JEE_RunGetStdOut("""" svclPath """ /Stdout /GetPercent """ appName """")
    currentVol := Trim(output)
    if (currentVol is not Number)
        currentVol := -1

    ; If vol was passed, set it
    if (vol != "")
        Run, %ComSpec% /c ""%svclPath%" /SetVolume "%appName%" %vol%", , Hide
...
SetWindowMute(winName := "A", shouldMute := "toggle") {
    ; Resolve PID from window
    WinGet, winPid, PID, %winName%
    if !winPid {
        WinGet, winPid, PID, ahk_id %winName%
        if !winPid
            WinGet, winPid, PID, ahk_pid %winName%
        if !winPid {
            MsgBox % "Could not get PID for: " winName
            return -2
        }
    }
    ...
    if !volume := GetVolumeObject(winPid) {
        ; MsgBox % "Could not get volume object for PID: " winPid
        return -1
    }

    VA_ISimpleAudioVolume_GetMute(volume, isMuted)

    if (shouldMute == "toggle")
        newMute := !isMuted
    else
        newMute := !!shouldMute  ; ensure boolean (non-zero → true)

    VA_ISimpleAudioVolume_SetMute(volume, newMute)
    ObjRelease(volume)

    return newMute ? "Muted" : "Unmuted"
}

VA_SetWindowVol(winName:="A",vol:="n"){
	winGet,winPid,PID,% winName
	if !winPid
		winGet,winPid,PID, ahk_id %winName%
		if !winPid
...
;Only used to run cmd/powershell commands hidden
MOD_JEE_RunGetStdOut(vTarget, vSize:="")
{
...
;Needs exact name rn for fallback
VA_SaferSetDefaultEndpoint(device_desc, role)
{
    ...
    ; Confirm if it's default now
    current := VA_GetDeviceName(VA_GetDevice()) 
	if (InStr(current, device_desc, true))
        return hr = 0

    ; Fallback using PowerShell
    device := VA_GetDevice(device_desc)
    if (device && VA_IMMDevice_GetId(device, deviceID) = 0) {
        ObjRelease(device)
        psCmd := "powershell -Command " . Chr(34) . "Set-AudioDevice -ID '" . deviceID . "' -DefaultOnly | Out-Null" . Chr(34)
        try {
            MOD_JEE_RunGetStdOut(psCmd)
        }
    }
...
Getdevicesfromenum(device_type:=0)
{
	...
	; device_type(eRender) := playback = 0, eCapture = 1, eAll = 2
	; 0x1 = DEVICE_STATE_ACTIVE
	DllCall(NumGet(NumGet(IMMDeviceEnumerator+0)+3*A_PtrSize), "UPtr", IMMDeviceEnumerator, "UInt", device_type, "UInt", 0x1, "UPtrP", IMMDeviceCollection, "UInt")
	ObjRelease(IMMDeviceEnumerator)
	...
	For DeviceName, DeviceID in Devices
		List .= DeviceName . "`n", ObjRawSet(Devices2, A_Index, DeviceID)
	return List
}	
...
;specific app mute & vol
GetVolumeObject(Param := 0)
{
    static IID_IASM2 := "{77AA99A0-1BD6-484F-8BC7-2C654C9A9B6F}"
    , IID_IASC2 := "{bfb7ff88-7239-4fc9-8fa2-07C950BE9C6F}"
    ...
    ; Get session manager
    VA_IMMDevice_Activate(device, IID_IASM2, 0, 0, pSessionManager2)
    ObjRelease(device)
    if !pSessionManager2
        return

    ; Get session enumerator
    VA_IAudioSessionManager2_GetSessionEnumerator(pSessionManager2, pEnumerator)
    ObjRelease(pSessionManager2)
    if !pEnumerator
        return

    VA_IAudioSessionEnumerator_GetCount(pEnumerator, count)
    found := false

    Loop % count {
        VA_IAudioSessionEnumerator_GetSession(pEnumerator, A_Index - 1, pSession)
        if !pSession
            continue

        pSession2 := ComObjQuery(pSession, IID_IASC2)
        ObjRelease(pSession)

        if !pSession2
            continue

        VA_IAudioSessionControl2_GetProcessID(pSession2, spid)
        if (spid == Param) {
            pVolume := ComObjQuery(pSession2, IID_ISAV)
            found := true
        }
        ObjRelease(pSession2)

        if (found)
            break
    }

    ObjRelease(pEnumerator)
    return found ? pVolume : ""
}
